...
function createInjectorButton(text: string, html: string): void {
...
function createInjectorButtonAdvanced(
    text: string,
    html: string,
    b: HTMLButtonElement
): HTMLButtonElement {
...
let button1 = createButtonEvP("Botón Inyectado", "btn1", "clb", {
    event: "click",
    handler: () => { console.log("Botón 1 presionado"); }
});

let bx = createInjectorButtonAdvanced(
    "Inyectar HTML",
    "<p><strong>¡HTML inyectado!</strong></p>",
    button1
);

createInjectorButtonAdvanced(
    "2 Inyectar HTML",
    "<div style='background-color: #FF0000; padding: 20px; color: white;'>Este div tiene un fondo rojo.</div>",
    bx
);

let experimentBtn1 = createButtonEvP("Experimento: Multi-Componentes", "exp1", "clb", {
    event: "click",
    handler: () => {
        const appDiv = document.getElementById("App");
        if (appDiv) {
...
let chain1 = createButtonEvP("Chain 1", "chain1", "clb", {
    event: "click",
    handler: () => { console.log("Presionado: Chain 1"); }
});

let chain2 = createInjectorButtonAdvanced(
    "Chain 2 (inyecta Chain 1)",
    "",
    chain1
);

let chain3 = createInjectorButtonAdvanced(
    "Chain 3 (inyecta Chain 2)",
    "",
    chain2
);

let clearButton = createButtonEvP("Limpiar #App", "clear", "clb", {
    event: "click",
    handler: () => {
        const appDiv = document.getElementById("App");
        if (appDiv) {
